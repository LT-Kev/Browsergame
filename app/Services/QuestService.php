...
class QuestService {
    ...
    public function getQuestById(int $questId): ?array {
        ...
        return $this->db->selectOne($sql, [':id' => $questId]);
    }
    ...
    public function getPlayerQuests(int $playerId, ?string $status = null): array {
        if($status) {
            ...
            return $this->db->select($sql, [':player_id' => $playerId, ':status' => $status]);
        }
    ...
    public function startQuest(int $playerId, int $questId): array {
        $sql = "SELECT * FROM player_quests 
                ...
                LIMIT 1";
        $existing = $this->db->selectOne($sql, [':player_id' => $playerId, ':quest_id' => $questId]);
        ...
        $result = $this->db->insert($sql, [':player_id' => $playerId, ':quest_id' => $questId]);
    ...
    public function completeQuest(int $playerId, int $questId): array {
        $sql = "UPDATE player_quests SET status = 'completed', completed_at = NOW() 
                WHERE player_id = :player_id AND quest_id = :quest_id AND status = 'active'";
        $result = $this->db->update($sql, [':player_id' => $playerId, ':quest_id' => $questId]);
