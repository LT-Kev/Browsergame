...
if(!$playerId || !$admin->isAdmin($playerId)) {
    echo '<p style="color: #e74c3c;">Zugriff verweigert</p>';
    exit;
}

// Berechtigung prüfen
if(!$admin->hasPermission($playerId, 'view_logs')) {
    echo '<p style="color: #e74c3c;">Keine Berechtigung für Logs</p>';
    exit;
}
...
foreach($lines as $line) {
    if(strpos($line, '[ERROR]') !== false) echo '<span class="error">' . $line . '</span>' . "\n";
    elseif(strpos($line, '[WARNING]') !== false) echo '<span class="warning">' . $line . '</span>' . "\n";
    elseif(strpos($line, '[INFO]') !== false) echo '<span class="info">' . $line . '</span>' . "\n";
    elseif(strpos($line, '[CRITICAL]') !== false) echo '<span class="critical">' . $line . '</span>' . "\n";
    else echo $line . "\n";
}
?></pre>
<?php endif; ?>
