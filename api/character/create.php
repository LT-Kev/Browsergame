...
if ($player['character_created']) {
    $api->error('Character already created', 400);
}
...
try {
    ...
    if (!$data) {
        $api->error('Invalid JSON data', 400);
    }
    ...
    // Grundvalidierung
    if (!$raceId || !$classId) {
        $api->error('Invalid race or class', 400);
    }
    ...
    foreach ($allowedStats as $stat) {
        if (!isset($stats[$stat])) {
            $api->error("Missing stat: $stat", 400);
        }
        
        $val = intval($stats[$stat]);
        
        if ($val < 0 || $val > 10) {
            $api->error("Invalid points for $stat", 400);
        }
    ...
    if ($totalBonus > 10) {
        $api->error('Too many stat points distributed', 400);
    }
    ...
    if (!$race || !$class) {
        $api->error('Race or class not found', 404);
    }
    ...
    $db->rollback();
    $api->error('Failed to create character: ' . $e->getMessage(), 500);
}